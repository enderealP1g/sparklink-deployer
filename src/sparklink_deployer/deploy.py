...
def install(config: DeploymentConfig, project_root: Path, assume_yes: bool) -> str:
    ...
    transaction.add_note("alpha install on a fresh Ubuntu 24.04 host")
    try:
        ...
        installed_versions = {}
        for key, binary_name in (("xray", "xray"), ("sing_box", "sing-box"), ("wireproxy", "wireproxy")):
        ...
        public_ranges = fetch_cloudflare_ranges()
...
def _apply_firewall(config: DeploymentConfig, ranges: dict[str, list[str]], transaction: Transaction) -> None:
    ...
    rules = [
        ["ufw", "allow", f"{config.host.ssh_port}/tcp", "comment", "SparkLink SSH preserve"],
        ["ufw", "allow", f"{config.ports.reality}/tcp", "comment", "SparkLink REALITY"],
        ["ufw", "allow", f"{config.ports.anytls}/tcp", "comment", "SparkLink AnyTLS"],
    ]
    ...
    if config.firewall.allow_http_for_acme:
...
def _validate_before_activation() -> None:
