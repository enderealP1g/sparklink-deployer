...
2. If the selected profile includes CDN, create its hostname as `DNS only` and point it to
   the same VPS. Otherwise no CDN record or Cloudflare rule is required.
3. Confirm the selected DNS records resolve to the VPS native IPv4. Do not enable the CDN
   origin-port rule yet.
